...
const Template13 = () => {
  ...
  const { resumeData, setResumeData, sectionOrder, setSectionOrder } = useResume();
...
// ⭐ LEFT COLUMN DYNAMIC SECTIONS (NO UI / CSS CHANGE)
            ...
            >
              ...
              {editMode ? (
                <div className="space-y-3">
                  ...
                  />
                  <textarea
                    value={Array.isArray(exp.accomplishment)
...
// ⭐ RIGHT COLUMN DYNAMIC (FULL UI + CSS SAME)

// ⭐ RIGHT COLUMN - FULL DYNAMIC (UI & CSS EXACT SAME)

const rightSections = {

  education: () => (
    (editMode || (localData.education && localData.education.length > 0)) && (
      <div>
        <SectionHeading title="Education" icon={GraduationCap} />
        <div className="space-y-4 mb-8">
          {(localData.education || []).map((edu, i) => (
            <div key={i} className="bg-white rounded-xl p-4 shadow-sm relative">
              ...
              {editMode ? (
                <div className="space-y-2">
                  ...
                  <input
                    value={edu.duration}
                    onChange={(e) =>
                      handleNestedChange("education", i, "duration", e.target.value)
                    }
                    className="w-full p-1 border rounded"
                    placeholder="Year"
                  />
  ...
  certifications: () => (
    (editMode || (localData.certifications && localData.certifications.length > 0)) && (
      <div>
              ...
              {editMode ? (
                <div className="space-y-2">
                  <input
                    value={cert.title}
                    onChange={(e) =>
                      handleNestedChange("certifications", i, "title", e.target.value)
                    }
                    className="w-full font-bold p-1 border rounded"
                  />
                  <input
                    value={cert.issuer}
                    onChange={(e) =>
                      handleNestedChange("certifications", i, "issuer", e.target.value)
                    }
                    className="w-full p-1 border rounded"
                  />
                  <input
                    value={cert.date}
                    onChange={(e) =>
                      handleNestedChange("certifications", i, "date", e.target.value)
                    }
                    className="w-full p-1 border rounded"
                  />
                </div>
                ...
                <>
                  <h3 className="text-md font-bold text-gray-800">{cert.title}</h3>
  ...
  courses: () => (
    (editMode || (localData.courses && localData.courses.length > 0)) && (
      <div>
        ...
        <div className="space-y-4 mb-8">
          {(localData.courses || []).map((course, i) => (
            <div key={i} className="bg-white rounded-xl p-4 shadow-sm relative">
              ...
              {editMode && (
                <button
                  onClick={() => handleRemoveItem("courses", i)}
                  className="absolute top-1 right-1 text-red-500"
                >
                  <Trash2 size={14} />
                </button>
              ...
              {editMode ? (
                <div className="space-y-2">
                  <input
                    value={course.title}
                    onChange={(e) =>
                      handleNestedChange("courses", i, "title", e.target.value)
                    }
                    className="w-full font-bold p-1 border rounded"
                  />
                  <input
                    ...
                    onChange={(e) =>
                      handleNestedChange("courses", i, "description", e.target.value)
                    }
                    className="w-full p-1 border rounded"
                  />
                </div>
                ...
                <>
                  <h3 className="text-md font-bold text-gray-800">{course.title}</h3>
  ...
  languages: () => (
    (editMode || (localData.languages && localData.languages.length > 0)) && (
      <div>
  ...
  interests: () => (
    (editMode || (localData.interests && localData.interests.length > 0)) && (
      <div>
        ...
        >
              ...
              {(editMode || localData.email) && (
                <div className="flex items-center gap-2 min-w-max">
                  ...
                  {editMode ? (
                    ...
                    <a href={`mailto:${localData.email}`} style={{color:"inherit"}}>
                      {localData.email}
                    </a>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2 min-w-max">
                  ...
                  {editMode ? (
                    <input
                      ...
                      className="bg-transparent border-b border-gray-600 text-white outline-none"
                    />
              ...
              {(editMode || localData.linkedin) && (
                <div className="flex items-center gap-2 min-w-max">
                  ...
                  {editMode ? (
                    ...
                    <a href={localData.linkedin} target="_blank" rel="noopener noreferrer">
                      {localData.linkedin}
                    </a>
                  )}
                </div>
              ...
              {(editMode || localData.github) && (
                <div className="flex items-center gap-2 min-w-max">
                  ...
                  {editMode ? (
                    ...
                    <a href={localData.github} target="_blank" rel="noopener noreferrer">
                      {localData.github}
                    </a>
                  )}
                </div>
          ...
          <div className="flex flex-col md:flex-row">
            ...
            <div className="flex-1 p-8">
              {sectionOrder
                .filter(section =>
                  ["summary","experience","projects","achievements"].includes(section)
                )
                .map(section=>(
                  <div key={section}>{leftSections[section]?.()}</div>
                ))}
            </div>
            ...
            <div className="w-full md:w-80 bg-gray-50 p-8 border-l border-gray-200">
              {sectionOrder
                .filter(section =>
                  ["education","certifications","courses","skills","languages","interests"].includes(section)
                )
                .map(section=>(
                  <div key={section}>{rightSections[section]?.()}</div>
                ))}
            </div>
        ...
        <div className="mt-8 text-center">
          {editMode ? (
            <div className="flex gap-4 justify-center">
              <button onClick={handleSave} className="bg-green-600 text-white px-6 py-3 rounded-lg">
                Save Changes
              </button>
              <button onClick={handleCancel} className="bg-gray-500 text-white px-6 py-3 rounded-lg">
                Cancel
              </button>
            </div>
          ) : (
            <button onClick={()=>setEditMode(true)} className="bg-blue-600 text-white px-8 py-3 rounded-lg">
              Edit Resume
            </button>
          )}
        </div>
